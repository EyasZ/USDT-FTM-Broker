...
class TradingBot:
    def __init__(self, secrets ,budget=10000, chain_ids=None, interval=30, api_key="YOUR_API_KEY"):
        self.budget = budget
    ...
    def check_last_pulse(self, chain_id):
        try:
            last_pulse = self.one_inch_api.check_wallet_assets(self.wallet_address, chain_id)
            return last_pulse
    ...
    def manage_trading_dict(self, chain_name, chain_id):
        ...
        if last_pulse and self.trading_dict:
            ...
            for address, balance in last_pulse.items():
                ...
                if not token:
                    current_price_info = self.one_inch_api.get_swap_rate(address, network=chain_id)
                    time.sleep(1)
                    token_info = self.one_inch_api.get_token_info(address, chain_id)
                    time.sleep(1)
                    if current_price_info and 'price' in current_price_info:
                        current_price = int(current_price_info['price'])  # Ensure float for accurate calculations
                        market_cap = self.get_market_cap(address, chain_id)
                    token_name = token_info['name']
                    token_symbol = token_info['symbol']
                    decimals = token_info['decimals']
                    initial_score = self.calculate_initial_score(market_cap)
                    token = Token((address, chain_id, token_name, token_symbol, decimals), initial_score,
                                  current_price)
                ...
                if token.white_listed:
                    # Check if the token is in the trading_dict or is the native token
                    if address not in self.trading_dict and address != self.native_token:
                        # If not, mark this token for selling
                        to_sell.append((address, balance))
                    elif address in self.trading_dict:
                        # If it is in trading_dict, check if balance needs adjustment
                        desired_balance = self.trading_dict[address].desired_balance
                        if balance > desired_balance:
                            to_sell.append((address, balance - desired_balance))
                        elif balance < desired_balance:
                            to_buy.append((address, desired_balance - balance))
            ...
            for address, token in self.trading_dict.items():
                if address not in pulse_addresses and token.white_listed:
                    to_buy.append((address, token.desired_balance))
    ...
    def swap_token_for_native(self, token, chain, amount):
        tx_hash = self.one_inch_api.swap_tokens(self.wallet_address, self.private_key, token.id,
                                                self.native_token, amount, chain.endpoint)
        self.logging.info(f"swapped {token.id} for native currency with amount {amount}\n transaction hash = {tx_hash}")

    def swap_native_for_token(self, token, chain, amount):
        tx_hash = self.one_inch_api.swap_tokens(self.wallet_address, self.private_key,
                                                self.native_token, token.id, amount, chain.endpoint)
